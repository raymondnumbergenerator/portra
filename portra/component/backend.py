...
class Backend():
    ...
    def save_image(self, file):
        ...
        get_img_url(filename) should return the URL to this file.
        """
        return
...
class FileBackend(Backend):
    ...
    def __init__(self):
        self.IMAGES_PATH = app.config['STORAGE_BACKEND']['img_path']
        self.METADATA_PATH = app.config['STORAGE_BACKEND']['met_path']
    ...
    def get_img_info(self, filename):
        ...
        info_path = os.path.join(self.METADATA_PATH, info_filename)

        # Regenerate the metadata file if it is missing.
        if not os.path.isfile(info_path):
            info = self.__img_info__(self.__img_path__(filename))
            pickle.dump(info, open(os.path.join(self.METADATA_PATH, info_filename), 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def save_image(self, file):
        ...
        file.save(os.path.join(self.IMAGES_PATH, filename))
    ...
    def __serialize_xmp__(self, xmp):
        return xmp_str
